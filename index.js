// ================ ASSIGNMENT 1 =================\\
  ...
  const numbers1 =  [2, 3, 7, 23, 9, 4, 675, 8, 0, -1, 8, 45, -69, 30, 55]

  const backReverse = numbers1.reverse((num , num1) => num1.length - num1.length)
...
// ================ ASSIGNMENT 3 =================\\
 ...
 const getNumber = [2, 3, 7, 23, 9, 4, 675, 8, 0, -1, 8, 45, -69, 30, 55]

 let max = getNumber[0] , min = getNumber[0]
 for(let i = 0;   i < getNumber.length; i++) {
 i
     if (getNumber[i] > max) { max = getNumber[i]; }

     if(getNumber[i] < min) { min = getNumber[i]; }
 }
